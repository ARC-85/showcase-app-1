...
const val PROJECT_JSON_FILE = "projects.json" // Supplementary Project JSON file created but used as backup, not called
...
val projectListType: Type = object : TypeToken<ArrayList<NewProject>>() {}.type // Secondary list created based on Project model
...
class PortfolioJSONStore(private val context: Context) : PortfolioStore {
    ...
    override fun createProject(project: NewProject, portfolio: PortfolioModel) {
        ...
        serializeProjects() // Adding project to project JSON file
    ...
    override fun updateProject(project: NewProject, portfolio: PortfolioModel) {
        var foundProject: NewProject? = projects.find { p -> p.projectId == project.projectId } // Within the projects list, find the project that matches the id of the passed project
        // If the project is found, update all the data for that project
        if (foundProject != null) {
            ...
            serializeProjects() // Update project on the project JSON file
        }
    ...
    override fun deleteProject(project: NewProject, portfolio: PortfolioModel) {
        ...
        serializeProjects() // Update the project JSON file
        var foundPortfolio: PortfolioModel? = portfolios.find { p -> p.id == portfolio.id }
        if (foundPortfolio != null) { // If the portfolio is found...
            if (foundPortfolio.projects != null) { // And the portfolio has projects (as expected)
                var projectIdList = arrayListOf<Long>() // Create a arrayList variable for storing project IDs
